...
filename='C:\Users\ND\OneDrive - Arizona State University\SEOF_Raytracing 082823\data\medswings10cm041525_water.xlsx';%
...
SystemParam.angnum=21;               %number of angles of led emission (must be less than raynum, below)
...
nfib=1.5+([2.5E-07,3.75E-07,5E-07]).*1i;%SystemParam.n1;%1.5+([1E-07,1.2E-07,1.5E-07,2.5E-07,5E-07].*1i);%description of the changes made in this file
...
max_scatter_ang=[pi/15,pi/18];%SystemParam.scat_ang_max;%[pi/3,pi/7,pi/18];%SystemParam.scat_ang_max;%[pi/2.1,pi/6.75,pi/5,pi/8,pi/18];%[pi/6.75,pi/6.9,pi/7,pi/7.1,pi/7.25];%%linspace(pi/7.2,pi/5.8,7);%[pi/7.2,pi/6.8,pi/6.4];%linspace(pi/6,pi/2,5);%[pi/18,pi/15,pi/12,pi/9,pi/6,pi/4,pi/3,pi*2/5,pi*3/7,pi/2];% [pi/18,pi/6,pi/4,pi/3,pi/2];%
bounce_var=SystemParam.maxbounce;%[1,2,3,4,5];%
raysc_coeff=[0.7,0.725,0.75,0.775,0.8];%SystemParam.r_coeff;%[0.9,0.6,0.3];%[0.01,0.5,0.9];%[0.1:0.025:0.2,0.3];%SystemParam.r_coeff;%[0.1,0.125,0.15,0.175,0.2];%[0.1,0.25,0.5];%%coefficient for what percentage of the loss is attributed to scattering vs absorption
